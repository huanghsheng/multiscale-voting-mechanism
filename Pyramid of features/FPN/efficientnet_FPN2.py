...
import torchvision
import sys
from model_FPN2 import EfficientNet
...
def evaluate_accuracy(data_iter, net,device=None):
    ...
    f = open("//home//jinfei//PycharmProjects//pythoncodes//FPN_test time.txt", "a")
    t1 = time.time()
    with torch.no_grad():
        batch_count = 0
        for X, y in data_iter:
            ...
            # if isinstance(net, torch.nn.Module):
            #     net.eval() # 评估模式, 这会关闭dropout
            #     acc_sum += (net(X.to(device))[0].argmax(dim=1) == y.to(device)).float().sum().cpu().item()
            #     net.train() # 改回训练模式
            # else:
            #     if('is_training' in net.__code__.co_varnames): # 如果有is_training这个参数
            #         # 将is_training设置成False
            #         acc_sum += (net(X, is_training=False)[0].argmax(dim=1) == y).float().sum().item()
            #     else:
            #         acc_sum += (net(X)[0].argmax(dim=1) == y).float().sum().item()
...
def train(train_iter, test_iter, net, loss, optimizer, device, num_epochs):
    ...
    step_schedule = optim.lr_scheduler.StepLR(step_size=100, gamma=0.2, optimizer=optimizer)
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch+1)>190:
            torch.save(net.module.state_dict(), "./weights/b0_FPN2-{}.pth".format(epoch+1))
...
net._fc1 = nn.Linear(50, 5)
pretext_model = torch.load('./pth/b0_disease_model_FPN2_400-300-2.pth')
net_dict = net.state_dict()
state_dict = {k: v for k, v in pretext_model.items() if k in net_dict.keys()}
net_dict.update(state_dict)
net.load_state_dict(net_dict)
...
torch.save(net.module.state_dict(), "///home//jinfei//PycharmProjects//pythoncodes//b0_chidu//b0_disease_model_FPN2_400-300-2.pth")