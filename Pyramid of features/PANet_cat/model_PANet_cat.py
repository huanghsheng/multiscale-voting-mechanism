...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None):
        ...
        self._fcx3 = nn.Linear(1680, 100)
        self._fcx31 = nn.Linear(100, 5)

        self._fcx4 = nn.Linear(432, 50)
        self._fcx41 = nn.Linear(50, 5)

        # self._fc3 = nn.Linear(15360, 1000)
        self._fc2 = nn.Linear(1152, 100)
        self._fc1 = nn.Linear(100, self._global_params.num_classes)
        ...
        # Last Smooth layers
        self.lastsmooth1 = nn.Conv2d(48, 48, kernel_size=3, stride=1, padding=1)
        self.lastsmooth2 = nn.Conv2d(72, 72, kernel_size=3, stride=1, padding=1)
        self.lastsmooth3 = nn.Conv2d(96, 96, kernel_size=3, stride=1, padding=1)
        ...
        self.downlatlayer1 = nn.Conv2d(24, 24, kernel_size=3, stride=2, padding=1)
        self.downlatlayer2 = nn.Conv2d(48, 48, kernel_size=3, stride=2, padding=1)
        self.downlatlayer3 = nn.Conv2d(72, 72, kernel_size=3, stride=2, padding=1)
    def _upsample_add(self, x, y):
        _, _, H, W = y.size()
        return F.upsample(x, size=(H, W), mode='bilinear') + y

    def _downsample_cat_x3(self, x, y):
        x = self.downlatlayer1(x)
        _, _, H, W = y.size()
    ...
    def _downsample_cat_x4(self, x, y):
        x = self.downlatlayer2(x)
        _, _, H, W = y.size()
        # x=x[torch.arange(x.size(2))!=1]
        x=x[:,:,:H,:]
        # x=F.interpolate(x, scale_factor=0.5)
    ...
    def _downsample_cat_x(self, x, y):
        x = self.downlatlayer3(x)
        _, _, H, W = y.size()
        # x=x[torch.arange(x.size(2))!=1]
        x=x[:,:,:,:W]
        # x=F.interpolate(x, scale_factor=0.5)
    ...
    def forward(self, inputs):
        ...
        l3=self._downsample_cat_x3(p2,p3)
        # print(f'l3:{l3.shape}')
        p3=self.lastsmooth1(l3)
        # print(f'p3:{p3.shape}')
        l4=self._downsample_cat_x4(p3,p4)
        p4 = self.lastsmooth2(l4)
        # print(f'p4:{p4.shape}')
        l5 = self._downsample_cat_x(p4, p5)
