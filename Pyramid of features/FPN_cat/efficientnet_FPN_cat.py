...
import torchvision
import sys
from model_FPN_cat import EfficientNet
...
#                 y_max3 = y_hat3[i].max()
#                 y_max4 = y_hat4[i].max()
#                 c = [y_max, y_max1, y_max2, y_max3, y_max4]
#                 fin_max = c.index(max(c))
#                 # if fin_max == 1:
#                 #     y_hat[i] = y_hat1[i]
#                 if fin_max == 2:
#                     y_hat[i] = y_hat2[i]
...
#
#         f1.write('epoch %d, train loss %.4f, train acc %.3f, test loss %.4f, test acc %.3f'
#               % (epoch + 1, train_l_sum / batch_count, train_acc_sum / n,test_l_sum, test_acc_sum)+'\n')
#
...
def evaluate_accuracy(data_iter, net,device=None):
    if device is None and isinstance(net, torch.nn.Module):
        ...
        device = list(net.parameters())[0].device
    acc_sum,test_l_sum , n = 0.0,0.0, 0
    f = open("//home//jinfei//PycharmProjects//pythoncodes//FPN_cat_test time.txt", "a")
    t1 = time.time()
    with torch.no_grad():
        batch_count = 0
        for X, y in data_iter:
...
def train(train_iter, test_iter, net, loss, optimizer, device, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        # if (epoch+1):
        #     torch.save(net.module.state_dict(), "./weights/b0_FPN_cat-{}.pth".format(epoch+1))
...
net._fc1 = nn.Linear(50, 5)
pretext_model = torch.load('./pth/b0_disease_model_FPN_cat_400-300-2_1000~1200epo.pth')
net_dict = net.state_dict()
state_dict = {k: v for k, v in pretext_model.items() if k in net_dict.keys()}
net_dict.update(state_dict)
net.load_state_dict(net_dict)
...
torch.save(net.module.state_dict(), "///home//jinfei//PycharmProjects//pythoncodes//b0_disease_model_FPN_cat_400-300-2_1201~1210epo.pth")