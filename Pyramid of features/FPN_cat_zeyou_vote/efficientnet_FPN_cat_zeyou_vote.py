...
import torchvision
import sys
from model_FPN_cat_zeyou_vote import EfficientNet
...
def evaluate_accuracy(data_iter, net,device=None):
    ...
    f = open("//home//jinfei//PycharmProjects//pythoncodes//FPN_cat_zeyou_vote①_test time.txt", "a")
    t1 = time.time()
    with torch.no_grad():
        batch_count = 0
        for X, y in data_iter:
            ...
            y_hat_sum = y_hat + y_hat2 + y_hat3 + y_hat4
            y_hat_sum = y_hat_sum.cpu().numpy()
            ...
            #y_array是统计各尺度的预测标签，为下一步投票做准备
            y_array=np.zeros((180,4))
            y_pre=np.zeros((180))
            for i in range(batch_size):
                y_array[i][0] = y_hat[i].argmax()
                y_array[i][1] = y_hat2[i].argmax()
                y_array[i][2] = y_hat3[i].argmax()
                y_array[i][3] = y_hat4[i].argmax()

            if batch_size!=180:
                y_array=y_array[0:batch_size,]
                y_pre = y_pre[0:batch_size]
            # y_array=torch.from_numpy(y_array)
            # y_array=y_array.to(device)
            y_array = y_array.astype(int)

            ###vote1
            # Voting with probability summation
            # for i in range(batch_size):
            #     y_pre[i] = np.argmax((np.bincount(y_array[i])))

            ###vote2
            # Voting with the finest level
            # for i in range(batch_size):
            #     y_pre[i]=np.argmax(y_hat_sum[i])

            ###vote3
            #Voting with the maximum probability
            for i in range(batch_size):
                # y_pre[i] = np.argmax((np.bincount(y_array[i])))
                if (len(get_count_max_ele(y_array[i])))==1:
                    y_pre[i]=np.argmax((np.bincount(y_array[i])))
                else:
                    y_pre[i]=np.argmax(y_hat_sum[i])
                    # y_pre[i] = y_hat[i].argmax()

            # y_pre=np.argmax((np.apply_along_axis(np.bincount, 1, y_array)),axis=1)
            y_pre = torch.from_numpy(y_pre)
            if isinstance(net, torch.nn.Module):
                net.eval()  # 评估模式, 这会关闭dropout
                acc_sum += (y_pre.to(device) == y.to(device)).float().sum().cpu().item()
                net.train()  # 改回训练模式
            else:
                if ('is_training' in net.__code__.co_varnames):  # 如果有is_training这个参数
                    # 将is_training设置成False
                    net(X, is_training=False)
                    acc_sum += (y_pre == y).float().sum().item()
...
def train(train_iter, test_iter, net, loss, optimizer, device, num_epochs):
    ...
    f1 = open("//home//jinfei//PycharmProjects//pythoncodes//b0_chidu//results.txt", "a")
    f2 = open("//home//jinfei//PycharmProjects//pythoncodes//b0_chidu//results data.txt", "w")
    start = time.time()
    test_acc_max_l = []
    test_l_min_l = []
    train_acc_max_l = []
    train_l_min_l=[]
    step_schedule = optim.lr_scheduler.StepLR(step_size=400, gamma=0.2, optimizer=optimizer)
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch+1)>490:
            torch.save(net.module.state_dict(), "./weights/b0_vote-{}.pth".format(epoch+1))
    ...
    f = open("//home//jinfei//PycharmProjects//pythoncodes//b0_chidu//b0_results.txt", "a")
...
# net._fc1 = nn.Linear(50, 5)

net._fc1 = nn.Linear(50, 5)
pretext_model = torch.load('./pth/b0_disease_model_FPN_cat_zeyou_vote_400-300-2_1200epo_LR0.01~0.002.pth')
net_dict = net.state_dict()
state_dict = {k: v for k, v in pretext_model.items() if k in net_dict.keys()}
net_dict.update(state_dict)
net.load_state_dict(net_dict)
...
torch.save(net.module.state_dict(), "///home//jinfei//PycharmProjects//pythoncodes//b0_chidu//b0_disease_model_FPN_cat_zeyou_vote_400-300-2.pth")